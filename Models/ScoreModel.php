...
class ScoreModel extends Database implements IModel
{
  ...
  {
    return $this->delete("DELETE FROM note_scores WHERE id_item = ?", ["i", $id]);
  }
  ...
  public function modify($paramsArray)
  {
    ...
    if (is_object($existingScore) && empty((array)$existingScore)) {
      ...
      $this->insert(
        "INSERT INTO note_scores (id_item, id_user, score) VALUES (?, ?, ?)",
        ["iii", $id, $user_id, $score]
      );
    } else {
      // if it exists, update the score
      $this->update(
        "UPDATE note_scores SET score = ? WHERE id_item = ? AND id_user = ?",
